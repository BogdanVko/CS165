...
public class ResizingArrayQueue<E> extends ArrayQueue<E> implements IQueue<E>{
    ...
    protected void resizeArrayTo(int newSize) {
        ...
        if (newSize <= this.size()) {throw new RuntimeException();}
        E[] newElements = super.newArray(newSize);
        for (int i = removed; i < added; i++) {
        	newElements[i% newElements.length] = elements[i% elements.length];
        }
    ...
    public boolean offer(E e) {
        ...
        if (this.size() == elements.length) {
        	if(maxSize < elements.length*2) {
        		resizeArrayTo(maxSize);
        		
        		
        	}else {
        		resizeArrayTo(elements.length*2);
    ...
    public E poll() {
         ...
         if (this.size() < elements.length/4){
        	 if(minSize > elements.length/2) {
        		 resizeArrayTo(minSize);
        	 }else {
        		 resizeArrayTo(elements.length/2);
        	 }
         }
